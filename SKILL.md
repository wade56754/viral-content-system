...
McKee 叫裂隙，Sugarman 叫好奇心种子，某知名知识博主叫先破后立，程前叫谜题。
6本写作经典 + 某知名知识博主16篇 + [作者] 实战数据，收敛到同一个点。
...
| 给了原话/语音/碎片想法 | → 共创模式 | 保护 [作者] 原话，AI 补充扩展 |
...
2. 读 `[workspace]/data/revision-rules.md` — 改稿规律
3. 读 `[workspace]/data/used-weapons.md` — 排除已用弹药

### 写后（Phase 9 之后）
4. 保存 AI 原稿到 `data/draft-archive/YYYYMMDD-{主题}-原稿.md`
5. [作者] 改完终稿后，diff 原稿 vs 终稿：
...
Pass 2（L4+L5）：风格化 + 去AI化
  → 读 references/author-style-dna.md 做风格改写
...
```
按顺序读取，不可省略：
1. [workspace]/IDENTITY.md       → [作者] 是谁
2. [workspace]/VOICE_DNA.md      → 语言DNA
3. [workspace]/AUDIENCE.md       → 受众画像 v2
4. [workspace]/PLATFORM_RULES.md → 推特算法权重
...
```
Step 1：读 references/author-style-dna.md，对照改写全文
...
```
Step 1：读 [skills]/content-review/SKILL.md
...
- [作者] 改完后 diff → 更新 gotchas + revision-rules + used-weapons
...
| `references/author-style-dna.md` | [作者]风格DNA | Phase 5 (Pass 2) |
...
| 文件 | 路径 |
|------|------|
| [作者] 身份宪法 | `[workspace]/IDENTITY.md` |
| 语言DNA | `[workspace]/VOICE_DNA.md` |
| 受众画像v2 | `[workspace]/AUDIENCE.md` |
| 平台规则v2 | `[workspace]/PLATFORM_RULES.md` |
